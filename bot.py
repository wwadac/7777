...
async def admin_broadcast(call: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_message(call.message,
...
async def admin_ban_do(message: Message, state: FSMContext):
    ...
    try:
        ...
        pass
    await safe_send_message(message.chat.id, f"✅ Пользователь <code>{uid}</code> заблокирован.", parse_mode="HTML")
...
async def admin_unban_do(message: Message, state: FSMContext):
    ...
    unban_user(uid)
    await state.clear()
    await safe_send_message(message.chat.id, f"✅ Пользователь <code>{uid}</code> разблокирован.", parse_mode="HTML")
...
async def admin_mute_do(message: Message, state: FSMContext):
    ...
    await state.clear()
    await safe_send_message(message.chat.id, f"✅ Пользователь <code>{uid}</code> замучен на {minutes} мин.", parse_mode="HTML")
...
async def admin_unmute_do(message: Message, state: FSMContext):
    ...
    unmute_user(uid)
    await state.clear()
    await safe_send_message(message.chat.id, f"✅ Пользователь <code>{uid}</code> размучен.", parse_mode="HTML")
...
async def add_chan_title(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
...
async def add_chan_url(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
...
async def add_chan_id_msg(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    if not message.text.lstrip('-').isdigit():
    ...
    await state.clear()
    await safe_send_message(message.chat.id, f"✅ Канал <b>{data['chan_title']}</b> добавлен!", parse_mode="HTML")
...
async def admin_rm_chan(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return await call.answer("🚫", show_alert=True)
    chan_db_id = int(call.data.split("_")[3])
...
async def admin_check_user_start(call: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="❌ Отмена", callback_data="admin_panel"))
    await safe_edit_message(call.message, "🔍 Введите ID пользователя для проверки:", reply_markup=kb.as_markup())
...
async def admin_check_user_do(message: Message, state: FSMContext):
    ...
    await state.clear()
    if not u:
        await safe_send_message(message.chat.id, f"❌ Пользователь <code>{uid}</code> не найден.", parse_mode="HTML")
        return
    muted = is_muted(uid)
    text = (
